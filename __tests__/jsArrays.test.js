...
describe('isArray', () => {
  test('check whether an input is an array or not', () => {
    expect(isArray('notArray')).toEqual(false);
...
describe('arrayClone', () => {
  test('clone an array', () => {
    expect(arrayClone([1, 2, 3, 4])).toEqual([1, 2, 3, 4]);
    expect(arrayClone([1, 2, [3, 4]])).toEqual([1, 2, [3, 4]]);
  });
});
...
describe('firstArray', () => {
  test('get the first element of an array', () => {
    expect(firstArray([5, 6, 7, 8])).toEqual(5);
    expect(firstArray([8, 7, 6, 5])).toEqual(8);
  });
});

describe('lastArray', () => {
  test('get the last element of an array', () => {
    expect(lastArray([7, 9, 0, -2],3)).toEqual([9,0,-2]);
    expect(lastArray([7, 9, 0, -2],6)).toEqual([7,9,0,-2]);
  });
});

describe('myColour', () => {
  test('join all elements of an array into a string', () => {
    expect(myColour.toString()).toEqual('Red,Green,White,Black');
    expect(myColour.join()).toEqual('Red,Green,White,Black');
    expect(myColour.join('+')).toEqual('Red+Green+White+Black');
  });
});

describe('dash', () => {
  test('insert dashes (-) between each two even numbers', () => {
    expect(dash(12345)).toEqual('1-23-45');
    expect(dash(98765)).toEqual('9-87-65');
  });
});

describe('sortArray', () => {
  test('insert dashes (-) between each two even numbers', () => {
    expect(sortArray([ 3, 8, 7, 6, 5])).toEqual([ 3, 5, 6, 7, 8]);  
    expect(sortArray([ 3, 5, -3, 2, 1 ])).toEqual([ -3, 1, 2, 3, 5]);
  });
});

describe('mostFrequent', () => {
   test('find the most frequent item of an array', () => {
     expect(mostFrequent([3, 'a', 'a', 'a', 2, 3, 'a', 3, 'a', 2, 4, 9, 3])).toEqual('a');  
     expect(mostFrequent([ 3, 5, -3, 2, 1, -3, 2, -3])).toEqual(-3);
   });
});

describe('swapCase', () => {
   test('swap the case of each character', () => {
     expect(swapCase('The Quick Brown Fox')).toEqual('tHE qUICK bROWN fOX');  
     expect(swapCase('Super Mario Bros')).toEqual('sUPER mARIO bROS');
   });
});

describe('printNested', () => {
  test('prints the elements of the following array', () => {
    expect(printNested('Fox')).toEqual('Row 0: F Row 1: o Row 2: x ');  
    expect(printNested('Mario')).toEqual('Row 0: M Row 1: a Row 2: r Row 3: i Row 4: o ');
  });
});
